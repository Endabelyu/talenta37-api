...
export const loginSchema = z.object({
  username: z.string().min(5).openapi({
    description: "The email of the user.",
    example: "talentaAdmin",
  }),
  password: z
    .string()
...
export const registerSchema = loginSchema
  .extend({
    name: z.string().min(4).max(100).openapi({ example: "talenta" }),
    email: z.string().min(4).max(100).openapi({ example: "talenta@gmail.com" }),
    confirmPassword: z
      .string()
      .min(8, "Password must be at least 8 characters long")
      .max(255, "Password must not exceed 255 characters")
    ...
    roleId: z.string().default("staff").openapi({
